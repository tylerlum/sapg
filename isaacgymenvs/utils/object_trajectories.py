...
def get_hammer_trajectory(object_init_state, device="cuda"):
    ...
    swing_down_state[3:7] = (
        R.from_quat(swing_up_state[3:7]) * R.from_euler("z", 60, degrees=True)
    ).as_quat()
...
def get_hairbrush_trajectory(object_init_state, device="cuda"):
    ...
    rotate_180_state[3:7] = (
        R.from_quat(pick_up_state[3:7]) * R.from_euler("x", -180, degrees=True)
    ).as_quat()
...
def get_screwdriver_trajectory(object_init_state, device="cuda"):
    ...
    for cycle in range(num_cycles):
        for step in rotate_steps:
            ...
            rotate_state[3:7] = (
                R.from_quat(go_down_state[3:7]) * R.from_euler("x", step, degrees=True)
            ).as_quat()
...
def get_marker_trajectory(object_init_state, device="cuda"):
    ...
    trajectory_states = [
        ...
        go_down_state,
        go_down_down_state,
        go_down_down_down_state,
        go_down_down_down_down_state,
    ]
    #
    return torch.tensor(trajectory_states, dtype=torch.float32, device=device)
...
def get_eraser_trajectory(object_init_state, device="cuda"):
    ...
    rotate_90_state = pick_up_state.copy()
    rotate_90_state[3:7] = (
        R.from_quat(pick_up_state[3:7]) * R.from_euler("z", 90, degrees=True)
    ).as_quat()
...
def get_phone_trajectory(object_init_state, device="cuda"):
    ...
    rotate_90_again_state[3:7] = (
        R.from_quat(rotate_90_state[3:7]) * R.from_euler("y", 90, degrees=True)
    ).as_quat()
