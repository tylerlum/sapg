...
TOOLS_BY_CATEGORY = [
    ...
    ("stapler", ["staples_open", "sharpie_closed"]),
...
def main() -> None:
    ...
    spacing_y_within = 0.0  # No extra spacing within category (same row)
    ...
    for cat_idx, (category, tool_names) in enumerate(TOOLS_BY_CATEGORY):
        ...
        for col, tool_name in enumerate(tool_names):
            ...
            print(f"  [{idx+1}/{total_tools}] Loading: {tool_name} ({category}) at row {cat_idx}, col {col}")
