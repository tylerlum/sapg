...
# Manual selection: Specify which 2 tools to show per category
# Format: "category_name": ["tool_name_1", "tool_name_2"]
# Leave list empty [] to automatically select first 2 tools
SELECTED_TOOLS = {
    # Example:
    # "hammer": ["primitive_hammer", "hammer_001"],
    # "screwdriver": ["primitive_screwdriver", "screwdriver_001"],
    "hammer": ["hammer_2", "mallet"],
    "spatula": ["wooden_spatula", "spoon_spatula"],
...
def main() -> None:
    ...
    # Layout parameters
    spacing_y = 0.25  # Space between tools within a category
    section_spacing_x = 0.6  # Space between category rows (along X/depth)
    section_spacing_y = 0.8  # Space between category columns (along Y/horizontal)
    title_height = 0.15  # Height of title above tools
    x_offset = -0.6  # Shift all objects back in X direction
    y_offset = -0.3  # Base Y offset
    
    # Grid layout: 3 rows x 2 columns
    num_columns = 2
    num_rows = 3
    max_objects_per_category = 2  # Limit to 2 objects per category
    
    # Load tools by section (arranged in grid)
    category_index = 0
    for tool_type, tools in sorted(tools_by_type.items()):
        # Select tools based on manual dictionary or auto-select first 2
        if tool_type in SELECTED_TOOLS and SELECTED_TOOLS[tool_type]:
            # Manual selection: filter tools by specified names
            selected_names = SELECTED_TOOLS[tool_type][:max_objects_per_category]
            tools_limited = []
            for urdf_path, obj_path in tools:
                if urdf_path.stem in selected_names:
                    tools_limited.append((urdf_path, obj_path))
            # Maintain order from SELECTED_TOOLS dictionary
            tools_limited.sort(key=lambda t: selected_names.index(t[0].stem) if t[0].stem in selected_names else 999)
        else:
            # Auto-select: use first 2 tools (primitives first)
            tools_limited = sorted(tools, key=sort_primitive_tools_to_start)[:max_objects_per_category]
        
        print(f"\nLoading section: {tool_type} ({len(tools_limited)} tools, {len(tools)} total available)")
        if tool_type in SELECTED_TOOLS and SELECTED_TOOLS[tool_type]:
            print(f"  Manual selection: {[t[0].stem for t in tools_limited]}")
        else:
            print(f"  Auto-selected: {[t[0].stem for t in tools_limited]}")
        
        # Calculate grid position for this category
        row = category_index // num_columns
        col = category_index % num_columns
        
        # Calculate base position for this category (X=depth, Y=horizontal)
        category_x = x_offset + row * section_spacing_x
        category_y = y_offset + col * section_spacing_y
        
        # Add section title at the front of the section
        server.scene.add_label(
            f"/section_{tool_type}/title",
            text=f"{tool_type.upper().replace('_', ' ')}",
            position=(category_x, category_y - 0.15, title_height),
        )
        
        # Load each tool in this section (along Y axis)
        for i, (urdf_path, obj_path) in enumerate(tools_limited):
            y_pos = category_y + i * spacing_y
            
            tool_name = urdf_path.stem
            print(f"  [{i+1}/{len(tools_limited)}] Loading: {tool_name}")
            
            # Create frame for this tool
            server.scene.add_frame(
                f"/section_{tool_type}/{tool_name}",
                position=(category_x, y_pos, 0.0),
                show_axes=False,
                axes_length=0.1,
                axes_radius=0.001,
            )
    ...
    tools_loaded = min(category_index, num_rows * num_columns) * max_objects_per_category
    print(f"\nLoaded {tools_loaded} tools (max {max_objects_per_category} per category) in {min(category_index, num_rows * num_columns)} sections")
    print(f"Total available: {total_tools} tools in {len(tools_by_type)} categories")
