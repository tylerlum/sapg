...
def mujoco_mesh_to_trimesh(
    ...
    if texcoord_num > 0:
        ...
        if matid >= 0 and matid < mj_model.nmat:
            ...
            texid = int(
                mj_model.mat_texid[matid, int(mujoco.mjtTextureRole.mjTEXROLE_RGB)]
            )
            if texid < 0:
                texid = int(
            ...
            is_collision = (
                mj_model.geom_contype[geom_idx] != 0
                or mj_model.geom_conaffinity[geom_idx] != 0
            )
            if is_collision:
                color = np.array(
                    [204, 102, 102, 128], dtype=np.uint8
                )  # Red-ish for collision.
            else:
                color = np.array(
                    [31, 128, 230, 255], dtype=np.uint8
                )  # Blue-ish for visual.
        ...
        if matid >= 0 and matid < mj_model.nmat:
            ...
            if verbose:
                print(
                    f"Using default {'collision' if is_collision else 'visual'} color"
                )
