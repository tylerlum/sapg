...
# CONFIG
# -----------------------------
URDF_PATH = Path("/home/tylerlum/github_repos/sapg/assets/urdf/kuka_allegro_description_for_mujoco/kuka_allegro_touch_sensor.urdf")
assert URDF_PATH.exists(), f"URDF_PATH does not exist: {URDF_PATH}"
...
# -----------------------------
def main():
    # Load the URDF into MuJoCo model
    ...
    print(f"data.qvel shape: {data.qvel.shape}")
    print(f"data.ctrl shape: {data.ctrl.shape}")
    
    # List actuated joints
    actuated_joint_ids = [i for i in range(model.njnt) if model.jnt_type[i] != mujoco.mjtJoint.mjJNT_FIXED]
    print(f"Actuated joint indices: {actuated_joint_ids}")
    print(f"Number of actuated joints: {len(actuated_joint_ids)}")
    
    # Optional: print actuator names
    if hasattr(model, 'actuator_name'):
        print(f"Actuator names: {model.actuator_name}")


    # Initialize target joint positions: all ones
    qpos_target = np.ones_like(data.qpos)

    # Start viewer
    with mujoco.viewer.launch_passive(model, data) as viewer:
        print("Viewer launched. Press Ctrl+C to quit.")
        last_update = time.time()

        while viewer.is_running():
            now = time.time()
            if now - last_update > DT:
                # Basic PD control toward qpos_target
                qpos_err = qpos_target - data.qpos
                qvel_err = -data.qvel
                print(f"qpos_err.shape: {qpos_err.shape}")
                print(f"qvel_err.shape: {qvel_err.shape}")
                breakpoint()
