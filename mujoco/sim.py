...
assert len(INIT_JOINT_POS) == len(JOINT_NAMES) == len(ACTUATOR_NAMES) == N_JOINTS, f"len(INIT_JOINT_POS): {len(INIT_JOINT_POS)}, len(JOINT_NAMES): {len(JOINT_NAMES)}, len(ACTUATOR_NAMES): {len(ACTUATOR_NAMES)}, expected: {N_JOINTS}"

N_BODY_NAMES = 33
BODY_NAMES = ['world', 'base', 'link1', 'link2', 'link3', 'link4', 'link5', 'link6', 'link7', 'palmworld', 'palmpalm', 'palmff_base', 'palmff_proximal', 'palmff_medial', 'palmff_distal', 'palmff_tip', 'palmmf_base', 'palmmf_proximal', 'palmmf_medial', 'palmmf_distal', 'palmmf_tip', 'palmrf_base', 'palmrf_proximal', 'palmrf_medial', 'palmrf_distal', 'palmrf_tip', 'palmth_base', 'palmth_proximal', 'palmth_medial', 'palmth_distal', 'palmth_tip', 'table', 'object']
assert len(BODY_NAMES) == N_BODY_NAMES, f"len(BODY_NAMES): {len(BODY_NAMES)}, expected: {N_BODY_NAMES}"
...
class SimulatorConfig:
    ...
    def friction_array(self) -> np.ndarray:
        return np.array([self.friction.sliding_friction, self.friction.torsional_friction, self.friction.rolling_friction])
...
class Simulator:
    ...
    def _validate(self) -> None:
        assert JOINT_NAMES == self.joint_names[:N_JOINTS], f"JOINT_NAMES: {JOINT_NAMES}, self.joint_names: {self.joint_names[:N_JOINTS]}"
        assert ACTUATOR_NAMES == self.actuator_names[:N_JOINTS], f"ACTUATOR_NAMES: {ACTUATOR_NAMES}, self.actuator_names: {self.actuator_names[:N_JOINTS]}"
        assert BODY_NAMES == self.body_names, f"BODY_NAMES: {BODY_NAMES}, self.body_names: {self.body_names}"
    ...
    def set_robot_joint_pos_targets(self, q_targets: np.ndarray) -> None:
        assert q_targets.shape == (N_JOINTS,), f"q_targets.shape: {q_targets.shape}, expected: ({N_JOINTS},)"
    ...
    def run(self):
        ...
        while self.continue_running():
            ...
            if sleep_dt > 0:
                ...
                print(f"Simulation is running slower than real time, desired FPS = {1.0 / self.config.sim_dt:.1f}, actual FPS = {1.0 / loop_dt:.1f}")
