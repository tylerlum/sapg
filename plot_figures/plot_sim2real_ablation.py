...
RAW_DATA = {
    "No Random\nDisturbance": {
        "sim_mean": 98, "sim_std": 2,  # Works perfectly in easy sim
        "real_mean": 30, "real_std": 8 # Fails in real world
    },
    "No Delay": {
        "sim_mean": 95, "sim_std": 3,  # Works great in sim
        "real_mean": 45, "real_std": 6 # Latency kills it in real world
    },
    "No SysID": {
        "sim_mean": 90, "sim_std": 4,  # Decent sim
        "real_mean": 60, "real_std": 5 # Bad parameters hurt real
    },
    "Ours": {
        "sim_mean": 92, "sim_std": 3,
        "real_mean": 89, "real_std": 4 # The gap is tiny!
    }
}
...
def plot_sim_real_gap():
    ...
    ax.bar(x - width/2, sim_means, width, yerr=sim_stds, label="Simulation", 
           color=colors["Simulation"], capsize=4, edgecolor='black', linewidth=0.7)
    
    # Plot Real World Bars (Right)
    ax.bar(x + width/2, real_means, width, yerr=real_stds, label="Real World", 
           color=colors["Real World"], capsize=4, edgecolor='black', linewidth=0.7)
