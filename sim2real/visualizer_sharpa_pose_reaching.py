...
class RosSnapshot:
    ...
    joint_targets: Optional[np.ndarray]
    ...
    def make_copy_with_defaults(self) -> RosSnapshot:
        ...
        if self.joint_targets is None:
            warn_every("joint_targets is None", n_seconds=1.0)
            joint_targets = np.zeros(NUM_ARM_JOINTS + NUM_HAND_JOINTS)
        else:
...
class ViserVisualizationNode:
    ...
    def initialize_viser(self):
        ...
        self.robot_targets_viser = ViserUrdf(
            SERVER,
            robot_urdf_path,
            root_node_name="/robot/targets",
            mesh_color_override=GREEN_RGB,
        )
        ...
        for robot_targets_mesh in self.robot_targets_viser._meshes:
            ...
            robot_targets_mesh.opacity = 0.5
        ...
        self.robot_targets_viser.update_cfg(DEFAULT_Q)
    ...
    def joint_targets_callback(self, msg: JointState):
        """Callback to update the joint targets."""
        self.ros_snapshot.joint_targets = np.array(msg.position)
    ...
    def update_viser(self):
        ...
        joint_targets = ros_snapshot.joint_targets
        ...
        self.robot_targets_viser.update_cfg(joint_targets)
